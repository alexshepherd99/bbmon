...
- **The chart renders, but no real browser has loaded the page.** Verified on 2026-08-09 by running the shipped `dashboard.js` against the vendored ECharts bundle and live API data in Node with a jsdom DOM, rendering to SVG and looking at it. That covers the option structure, series building, null handling and the status line. Still unverified in a real browser: the canvas renderer, the poll timer, resize handling and the CSS layout — all of which G1's "dashboard reachable from a phone on the LAN" will exercise.
