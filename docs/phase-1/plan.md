...
- **Nothing in M2 has run on the Pi.** `bootstrap.sh`, `deploy.sh` and `update.sh` are shellcheck-clean and the units pass `systemd-analyze verify`, but no line of any of them has been executed on hardware, and the units have never been started at all. This is the same shape of risk M3 carried into G2 with the unrun Ookla binary, where the exercise found a real defect — so it is worth naming rather than assuming the scripts work. G1 is the check.
...
- **The latency chart re-animates on every poll.** Observed in a real browser on 2026-08-12: each five-second refresh redraws the lines left to right over about a second, which is distracting at that frequency. `refresh()` calls `setOption` with `replaceMerge: ["series"]`, so ECharts treats each poll as new series and replays the entry animation rather than transitioning the existing ones. **Deferred to a later session by agreement**; it is a display defect, not a data one. The likely fix is disabling the series entry animation, or merging data into the existing series instead of replacing them — worth checking which of those keeps the line transition smooth rather than making it jump.

- **A freshly started dashboard is blank for the first minute**, because nothing is written until the first 60s flush. Inherent to requirement 4's buffering. **Deferred to a later session, by agreement on 2026-08-09** — not urgent, but it is felt on every restart and every deploy, so it wants deciding before M7. The question to answer: is an immediate first flush on startup worth one extra write per service start? If yes it is a one-line change in `PingerService.run`, seeding `last_flush` so the first check passes.
