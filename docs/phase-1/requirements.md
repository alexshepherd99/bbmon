...
- **Reboot** — never actually invoked in dev, so the abstraction requirement 10 calls for has a no-op implementation as the dev default. The real implementation writes a trigger file that a `bbmon-reboot.path` unit watches, and is exercised on hardware at gate G3. (A sudoers rule was the recorded mechanism until M4, when it turned out `NoNewPrivileges=yes` makes sudo refuse to run at all — see `log.md`.)
...
- **LAN reachability** — Crostini does not expose ports to the LAN by default, so "usable from a phone on the LAN" (requirement 7) is verified on the Pi; dev testing is reachable from ChromeOS but not from the LAN. The container reaches the Pi over SSH fine — it sits on a host-only subnet and routes out through ChromeOS, so the Pi is two hops away rather than on the same link. An earlier note here claimed it could not reach the Pi at all; that was checked at M2 and was simply wrong.
